...
We design tests for the model from @sec-reproducibility. However, we will modify this model so that, instead of modifying a global class of parameter values, we **create instances of this class** and use it in our model.
...
For example, we expect that our model should fail if the simulation duration or the patient inter-arrival time were set to 0. This is tested using `test_zero_inputs`.

```{python}
{{< include tests/test_unittest.py >}}
```

When we run the test, we see there is one pass and one fail.
...
**Simulation duration** passes the test as SimPy has built in functionality requiring that `env.run()` is set to run for a time greater than the current simulation time (which is 0) - and so, it raises a ValueError and stops execution.

However, the **patient inter-arrival time** test fails. There is no ValueError raised by the model - in fact, we would find that the model will run infinetely, as it is just constantly generating new patients. Hence, this test was given a time limit using the `pytest-timeout` package, using which we can add the decorator `@pytest.mark.timeout(3)` which will end the test after 3 seconds.

From running these tests, we've therefore identified a **glitch** in the model. To address this, we could add error handling which raises an error for users if they try to input an inter-arrival time of 0. For example, we could add the following code to our `Model __init__` method:
...
* Back test from particular run of model

-->

### Further testing examples

For more test examples, check out the [Python DES RAP Template](https://github.com/pythonhealthdatascience/rap_template_python_des). Examples of other tests it includes are:
