...
print(f"inter-arrival time 1 {random.expovariate(1.0 / g.patient_inter):.2f}")
print(f"inter-arrival time 2 {random.expovariate(1.0 / g.patient_inter):.2f}")
print(f"reception consult time 1 {random.expovariate(1.0 / g.mean_reception_time):.2f}")
print(f"inter-arrival time 3 {random.expovariate(1.0 / g.patient_inter):.2f}")
```
...
First, we need `numpy`, plus the `Exponential` class from `sim-tools`.
...
class Model:
    ...
    def __init__(self, run_number):
        ...
        seeds = ss.spawn(4)
        self.patient_inter_arrival_dist = Exponential(mean=g.patient_inter,
                                                      random_seed=seeds[0])
        self.patient_reception_time_dist = Exponential(mean=g.mean_reception_time,
                                                       random_seed=seeds[1])
        self.nurse_consult_time_dist = Exponential(mean=g.mean_n_consult_time,
                                                   random_seed=seeds[2])
        self.doctor_consult_time_dist = Exponential(mean=g.mean_d_consult_time,
                                                   random_seed=seeds[3])

```

When we want to run multiple random processes at once (for example, having separate seeds for arrivals, reception, and consultation times), it's tempting to create them in a pattern like: seed1 = `run_number`, seed2 = `run_number * 2`, seed3 = `run_number * 3` and use these for each stream.

But here's the catch: if `run_number` ever happens to be zero, every single stream gets a seed of zero! Even if it's not zero, simple patterns like this can mean the random streams aren't truly independent - subtle correlations can sneak in, which can throw off your simulation results (even if the code seems fine). These mistakes are really easy to miss!

Instead, NumPy's `SeedSequence` gives you a safer approach. You give it one seed (like your `run_number`), and it will use this to generate separate, reliable seeds for each stream.

:::{.callout-warning}
Note that the value we pass to initialise the Exponential class here is **just the mean time**.

When we were using random.expovariate, we passed 1 divided by the mean time.
:::
